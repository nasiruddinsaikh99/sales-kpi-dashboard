        ...
        <main class="flex-grow p-4 md:p-8 max-w-7xl w-full mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
                <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Total Net GP</p>
                    <p class="text-3xl font-bold text-white">$<?= number_format($globalStats['total_net_gp'] ?? 0, 2) ?></p>
                    <p class="text-blue-400 text-xs mt-1">Lifetime Earnings</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Total Gross Profit</p>
                    <p class="text-3xl font-bold text-white">$<?= number_format($globalStats['total_gross_profit'] ?? 0, 2) ?></p>
                    <p class="text-emerald-400 text-xs mt-1">Total Revenue</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Total Payouts</p>
                    <p class="text-3xl font-bold text-white">$<?= number_format($globalStats['total_payout'] ?? 0, 2) ?></p>
                    <p class="text-amber-400 text-xs mt-1">Commission Paid</p>
                </div>
                <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg">
                    <p class="text-gray-400 text-xs font-bold uppercase tracking-wider mb-2">Total Records</p>
                    <p class="text-3xl font-bold text-white"><?= number_format($globalStats['total_records'] ?? 0) ?></p>
            ...
            <div class="bg-gray-800 p-6 rounded-xl border border-gray-700 shadow-lg mb-8">
                <h3 class="text-lg font-semibold text-white mb-6">Performance Trajectory</h3>
            ...
            <div class="flex justify-between items-center mb-8">
                <div>
                    <h2 class="text-3xl font-bold">Import History</h2>
                    <p class="text-gray-400 mt-1">Manage monthly data uploads and records.</p>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-700">
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center text-gray-500">
                                        <div class="flex flex-col items-center justify-center gap-2">
                                            <svg class="w-10 h-10 text-gray-600 mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                            ...
                            <?php else: ?>
                                ...
                                <tr class="hover:bg-gray-800/50 transition group">
                                    <td class="px-6 py-4 font-medium text-white"><?= htmlspecialchars($up['batch_name']) ?></td>
                                    <td class="px-6 py-4 text-indigo-300"><?= htmlspecialchars($up['for_month']) ?></td>
                                    <td class="px-6 py-4 text-sm text-gray-400"><?= htmlspecialchars($up['uploaded_at']) ?></td>
                                    <td class="px-6 py-4">
                                        <span class="px-2 py-1 rounded-full text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">Active</span>
                                    </td>
                                    <td class="px-6 py-4 text-right flex items-center justify-end gap-3">
                                        <a href="/sales-kpi-dashboard/admin/records?upload_id=<?= $up['id'] ?>" class="text-blue-400 hover:text-blue-300 text-sm font-medium hover:underline">View Records</a>
